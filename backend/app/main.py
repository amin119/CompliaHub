...
# fails cert verification against Voyage/Cohere/Gemini — some local network
# TLS interception isn't in certifi's bundled CA list. truststore repoints
# ssl at the OS's own trust store instead, the same fix `--native-tls` is for
# `uv` itself. Harmless on Linux (the worker doesn't import this module at all).
